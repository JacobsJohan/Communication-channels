...
% buildings are placed inside a 10m x 10m frame and the average electric 
% field amplitude at a constant distance from the central source is measured.
...
% Draw border with huge sigma for 100% absorption
...
field(1).EpsRel = draw_ellipse(field(1).EpsRel, 2, 4, 8, 1, 1, conf);
field(1).EpsRel = draw_ellipse(field(1).EpsRel, 2, 2, 4, 1, 1, conf);
...
Locations = draw_ellipse(Locations,1,5,5,4.3,4.3,conf);
