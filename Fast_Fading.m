...
%% Prepare video
v = VideoWriter('Vids\FF\FF','MPEG-4');
v.Quality = 100; 
open(v);
figure()
...
close(v)
%% Save Data
filename = ['FF_', datestr(now), '.mat'];
filename  = strrep(filename , ':', '_');%Save does not like ':'
save(filename)
...
plotBox('Box1',E_box1,conf,[70,1999]);
plotBox('Box2',E_box2,conf,[150 1999]);
plotBox('Box3',E_box3,conf,[200 1999]);
plotBox('Box4',E_box4,conf,[200 1999]);
...
xlabel('Frame number')
ylabel('E-field (V/m)')
title('FF at center of box 1')
subplot(2,2,2)
plot(center2)
xlabel('Frame number')
ylabel('E-field (V/m)')
title('FF at center of box 2')
subplot(2,2,3)
plot(center3)
xlabel('Frame number')
ylabel('E-field (V/m)')
title('FF at center of box 3')
subplot(2,2,4)
plot(center4)
xlabel('Frame number')
ylabel('E-field (V/m)')
title('FF at center of box 4')
suptitle('Fast fading at different points in the room')
%% Release path
rmpath(genpath(pwd))