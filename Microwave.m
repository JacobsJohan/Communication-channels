...
f = 900e06; %freq of source
sourceX = 1; %Horizontal
sourceY = 1;   %Vertical
Source = addSource( Source,conf,sourceY,sourceX,f,10*sin(2*pi*f*T) );
%% Gradual increase of refraction

for i=3:conf.y_length/0.5
field(1).EpsRel = draw_rectangle(field(1).EpsRel,i*0.5,10,0.5*i,20,0.5,conf);
end
%Drawing box around source
field(1).EpsRel = draw_rectangle(field(1).EpsRel,50000,0.9,1,0.05,0.2,conf);%left
field(1).EpsRel = draw_rectangle(field(1).EpsRel,50000,1,0.9,0.2,0.05,conf);%bottom
field(1).EpsRel = draw_rectangle(field(1).EpsRel,50000,1.1,1-0.05,0.05,0.2,conf);%right
field(1).EpsRel = draw_rectangle(field(1).EpsRel,50000,1-0.05,1.1,0.2,0.05,conf);%top
...
%% Prepare video
v = VideoWriter('Microwave','MPEG-4');
v.FrameRate = 8;
v.Quality = 100; 
open(v);
figure()
